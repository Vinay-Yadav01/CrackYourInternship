...
#define MAX_DIGITS 17
    ...
    // Helper: Check if number should be subtracted based on suffix and digit
    // constraints
    static bool checkSubtract(const string& num_str, ll num_digits,
    ...
    ll countValidNumbers(const string& number_str, ll idx, ll max_digits,
        ...
        for (ll digit = low; digit <= high; ++digit) {
            ...
            total += countValidNumbers(number_str, idx + 1, max_digits,
                                       new_tight, limit, suffix);
        }
    ...
    ll solveUpTo(const string& num_str, ll num_digits, int limit,
                 const string& suffix) {
        memset(dp, -1, sizeof(dp));
        ll result =
            countValidNumbers(num_str, 0, num_digits, true, limit, suffix);
...
public:
    long long numberOfPowerfulInt(long long start, long long finish, int limit,
        ...
        ll upto_finish =
            (finish >= suffix_val)
                ? solveUpTo(finish_str, finish_digits, limit, suffix)
                : 0;
        ll upto_start = (suffix_val < start)
                            ? solveUpTo(start_str, start_digits, limit, suffix)
                            : 0;
