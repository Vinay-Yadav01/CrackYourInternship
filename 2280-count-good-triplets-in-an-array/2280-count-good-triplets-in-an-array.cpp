...
public:
    long long goodTriplets(vector<int>& nums1, vector<int>& nums2) {
        ...
        for (ll i = 0; i < n; ++i)
            nums2_val_idx[nums2[i]] =
                i; // All elements are unique hence no collision on key

        // Step-2: Push the leftmost nums1 item in Segment Tree & Process all
        // items from idx 1
        updateSegTree(1, 0, n - 1, nums2_val_idx[nums1[0]]);

        ll count_good_triplets = 0;
        for (int i = 1; i < n - 1;
             ++i) { // Consider i as the middle item of triplet
            ...
            ll common_right_elements =
                (n - nums2_idx - 1) - uncommon_left_elements;
