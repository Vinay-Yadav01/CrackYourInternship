class Solution {
    ...
    int kahnsTopologicalSort(vector<int>& adj, vector<int>& indegree,
                             vector<bool>& visited, int source) {
        queue<int> q;
        q.push(source);
    ...
    int maxDepthBFS(vector<vector<int>>& adj, vector<bool>& orig_visited, int n,
                    int source, int avoid) {
...
public:
    int maximumInvitations(vector<int>& adj) {
        ...
        for (int i = 0; i < n; ++i) {
            if (indegree[i] == 0 and !visited[i]) {
                ...
                if (adj[adj[node]] == node) { // Cycle of length-2
                    int len_tail =
                        maxDepthBFS(reverse_adj, visited, n, node, adj[node]) -
                        1;
                    tot_tail_len +=
                        len_tail; // All 2-len cycles with tail can be together
                    visited[node] = false;
                }
            }
        }
        ...
        // Step-4: Find the best option of the 2 cases and return (All 2 size
        // cycles can be added)
